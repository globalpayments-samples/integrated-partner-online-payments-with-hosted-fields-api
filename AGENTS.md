...
# Expected: {"success":true,"data":{"apiKey":"..."}}
```

POST /process-payment cannot be tested with curl — the `payment_token` must be generated by the hosted fields iframes rendered in a browser. Use the frontend UI at `http://localhost:8000` to complete a payment via the hosted card form.

## Making Changes

All language implementations expose identical behavior. A change to one must be applied to all — each language in a separate commit. Do not modify shared files (`index.html`, `docker-compose.yml`, `Dockerfile.tests`) without confirming the change applies to every implementation.
